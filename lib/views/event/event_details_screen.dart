...
class _EventDetailsScreenState extends ConsumerState<EventDetailsScreen>
  ...
  void _showDeleteConfirmation() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content:
            const Text('Are you sure you want to delete this design image?'),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context),
  ...
  void _showIssueItemDialog() {
    showDialog(
      context: context,
      builder: (context) => _IssueItemDialog(
        eventId: _currentEventData['id'],
        onItemIssued: () {
...
class _IssueItemDialog extends ConsumerStatefulWidget {
  final int eventId;
  final VoidCallback onItemIssued;

  const _IssueItemDialog({
    required this.eventId,
    required this.onItemIssued,
  });

  @override
  ConsumerState<_IssueItemDialog> createState() => _IssueItemDialogState();
}

class _IssueItemDialogState extends ConsumerState<_IssueItemDialog> {
  final TextEditingController _quantityController = TextEditingController();
  final TextEditingController _notesController = TextEditingController();
  ...
  Future<void> _loadInventoryItems() async {
    ...
    try {
      final response =
          await ref.read(inventoryProvider.notifier).getInventoryItemsList();
      if (response['success'] == true) {
        setState(() {
          _inventoryItems =
        ...
        setState(() {
          _error = response['message'] ?? 'Failed to load inventory items';
          _isLoading = false;
        });
      }
    } catch (e) {
  ...
  Future<void> _submitIssuance() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(response['message'] ?? 'Item issued successfully')),
        );
        Navigator.of(context).pop();
        widget.onItemIssued();
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(response['message'] ?? 'Failed to issue item')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(
                        border: OutlineInputBorder(),
                        hintText: 'Enter notes for this issuance',
                      ),
                      maxLines: 3,
                    ),
                  ],
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: _isSubmitting ? null : _submitIssuance,
                      style: ElevatedButton.styleFrom(
                        backgroundColor: colorScheme.primary,
                        foregroundColor: colorScheme.onPrimary,
                      ),
